...
from llm_engine.tasks import build_task
...
def process_batch(task,model,batch):
    ...
    if status:
        ret = []
        for pd, od in zip(parsed_data,batch['datas']):
            merged = {**pd,**od}
            ret.append(merged)
    else:
        ret = [{"output": parsed_data,"datas":batch["datas"]}]
    return ret
...
if __name__ == '__main__':
    ...
    extension = os.path.splitext(args.data_file)[-1]
    if extension == ".json":
