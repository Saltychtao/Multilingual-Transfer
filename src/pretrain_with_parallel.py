...
class ModelArguments:
    ...
    pretrained_embed : Optional[str] = field(default=None)


@dataclass
class DataArguments:
    lm_train_file: str = field(default=None, metadata={"help": "Path to the training data."})
    parallel_train_file: str = field(default=None, metadata={"help": "Path to the training data."})
    ...
    sentswitch_ratio: Optional[float] = field(default=0)
    codeswitch_corpus_ratio: Optional[float] = field(default=0)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    bind_embeddings: Optional[str] = field(default=None)

    ba_strength: Optional[float] = field(default=0)

    trans_strength: Optional[float] = field(default=0)
    sentswitch_alignment_strength: Optional[float] = field(default=0)
...
def bind_embeddings(model,tokenizer,dict_file):
    ...
    with open(dict_file) as f:
        for line in f:
            ...
            src_idx = tokenizer(srcword)["input_ids"]
            tgt_idx = tokenizer(tgtword)["input_ids"]
            word_idxs.append((src_idx[0],tgt_idx[0]))
    
    for src_idx, tgt_idx in word_idxs:
        embed_tokens.weight.data[src_idx] = embed_tokens.weight.data[tgt_idx].clone()
        output_tokens.weight.data[src_idx] = output_tokens.weight.data[tgt_idx].clone()
...
def main():
    ...
    if training_args.bind_embeddings == "1":
        bind_embeddings(model,tokenizer,training_args.dict_file)

    training_args.sentswitch_ratio = data_args.sentswitch_ratio
    training_args.codeswitch_corpus_ratio = data_args.codeswitch_corpus_ratio

    data_module = make_data_module(data_args,model.config,tokenizer,type="lm_with_parallel")

    # Preprocessing the datasets.
    trainer = TrainerWithParallel(model=model,tokenizer=tokenizer,args=training_args, **data_module)
    trainer.train()
