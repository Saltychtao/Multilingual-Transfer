...
try:
    ...
    logger.warning(
        "flash_attn is not installed correctly. If you want to use memory_efficient_attention to accelerate training use the following command to install flash_attn\npip install flash_attn."
    )

try:
    from torch.nn.functional import scaled_dot_product_attention
    scaled_dot_product_attention_imported = True
except:
    scaled_dot_product_attention_imported = False
...
class MLP(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        mlp_state_dict["mlp_act"] = act
        return self.down_proj(act), mlp_state_dict
...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if scaled_dot_product_attention_imported:
            mask = ~attention_mask.isinf()
            attn_output = scaled_dot_product_attention(query_states,key_states,value_states,mask)
            attn_weights = None
        else:
...
class BaichuanLayer(torch.nn.Module):
    ...
    def forward(
        ...
        state_dict["resid_pre"] = residual
        hidden_states = self.input_layernorm(hidden_states)
        state_dict["attn_in"] = hidden_states
        # Self Attention
        ...
        state_dict["attn_out"] = hidden_states
        hidden_states = residual + hidden_states
        state_dict["resid_mid"] = hidden_states
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        state_dict["mlp_in"] = hidden_states
        hidden_states, mlp_state_dict = self.mlp(hidden_states)
        state_dict["mlp_out"] = hidden_states
        hidden_states = residual + hidden_states
        state_dict["resid_post"] = hidden_states
        outputs = (hidden_states,)

        if use_cache:
            outputs += (present_key_value,)

        return outputs, state_dict
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs, layer_state_dict = torch.utils.checkpoint.checkpoint(
                ...
                layer_outputs, layer_state_dict = decoder_layer(
                    hidden_states,
                    attention_mask=attention_mask,
                    past_key_value=past_key_value,
                    output_attentions=output_attentions,
            ...
            layer_state_dicts.append(layer_state_dict)
        ...
        state_dict = {
            k: torch.stack([v[k] for v in layer_state_dicts],dim=0) for k in layer_state_dicts[0]
        }
        hidden_states = self.norm(hidden_states)
        state_dict["post_final_ln"] =  hidden_states
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not return_dict:
            ...
            return ((loss,) + output, state_dict) if loss is not None else output,state_dict
