...
class GPTNeoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if has_flash_attn:
            attn_output, attn_weights = self._flash_attn(query,key,value)
        else:
...
class GPTNeoMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        state_dict["mlp_pre_act"] = hidden_states
        hidden_states = self.act(hidden_states)
        state_dict["mlp_post_act"] = hidden_states
        hidden_states = self.c_proj(hidden_states)
        hidden_states = self.dropout(hidden_states)
        return hidden_states, state_dict
...
class GPTNeoBlock(nn.Module):
    ...
    def forward(
        ...
        # residual connection
        state_dict["attn_out"] = attn_output
        hidden_states = attn_output + residual
        state_dict["resid_mid"] = hidden_states
        residual = hidden_states
        hidden_states = self.ln_2(hidden_states)
        state_dict["mlp_in"] = hidden_states
        feed_forward_hidden_states, mlp_state_dict = self.mlp(hidden_states)
        state_dict["mlp_out"] = feed_forward_hidden_states
        # residual connection
        hidden_states = residual + feed_forward_hidden_states
        state_dict["resid_post"] = hidden_states
        state_dict["mlp_pre_act"] = mlp_state_dict["mlp_pre_act"]
        state_dict["mlp_post_act"] = mlp_state_dict["mlp_post_act"]
        ...
        return outputs, state_dict  # hidden_states, present, (attentions, cross_attentions)
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs, block_state_dict = torch.utils.checkpoint.checkpoint(
                ...
                outputs, block_state_dict = block(
                    hidden_states,
                    layer_past=layer_past,
                    attention_mask=attention_mask,
            ...
            state_dict_list.append(block_state_dict)
        ...
        state_dict = {
            k: torch.stack([v[k] for v in state_dict_list],dim=0) for k in state_dict_list[0]
        }
        hidden_states = self.ln_f(hidden_states)
        # state_dict["final_ln"] = hidden_states
...
class GPTNeoForCausalLM(GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        transformer_outputs, state_dict = self.transformer(
            input_ids,
            past_key_values=past_key_values,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
        ...
        if not return_dict:
            output = (lm_logits,) + transformer_outputs[1:] + (state_dict,)
    ...
    def compute_consistency_loss(self, src_input_ids, tgt_input_ids):
        pass
