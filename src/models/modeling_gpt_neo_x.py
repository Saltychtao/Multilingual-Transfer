...
class SoLU(nn.Module):
    def forward(self,x):
        return nn.functional.softmax(x,dim=-1) * x
...
class GPTNeoXMLP(nn.Module):
    def __init__(self, config):
        ...
        if config.hidden_act == "solu":
            self.solu_ln = nn.LayerNorm(config.intermediate_size, eps=config.layer_norm_eps)
        else:
            self.solu_ln = nn.Identity()

    def forward(self, hidden_states):
        hidden_states = self.dense_h_to_4h(hidden_states)
        hidden_states = self.solu_ln(self.act(hidden_states))
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        if self.training:
            if use_cache and self.gradient_checkpointing:
                logger.warning(
